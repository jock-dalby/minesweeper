...
function showCell (evt) {
  ...
  if (evt.target.classList.contains('mine')) {
    revealMines()
    removeListeners()
    // displayMessage('BOOM! Pineapple Lumps for all you Kiwis out there!')
    ...
    document.getElementById('message').addEventListener('click', function (e) {
      ...
      if (e.target.id === 'no') {
        document.getElementById('message').innerHTML = `<div class="playAgain"><h3>Thanks for playing!</h3><button id="reset">RESET BOARD</button></div>`
      }
